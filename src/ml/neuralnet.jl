...
init_spacings = 0. #read_init_spacing_from_file("results/init_$(freq).txt")
...
function build_model()
    return Chain(Dense(length(optim_params.freq_range) * 2, 256),
                 Dropout(0.3),
                 x -> reshape(x, :, 1, 1),
                 Conv((3, ), 1 => 8, pad=(1, ), σ),
                 x -> maxpool(x, (2, )),
                 ...
                 Dense(#length(optim_params.freq_range) * 4,
                       ...
                       length(optim_params.freq_range)))
